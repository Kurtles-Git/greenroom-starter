...
export function CommandPalette({ shows, artists }: Props) {
  ...
  const go = useCallback(
    (item: ResultItem) => {
      if (item.kind === "show") {
        navigate({ to: "/shows/$id", params: { id: item.entry.id } });
      }
      setOpen(false);
    },
    [navigate],
      ...
      >
        <div className="flex items-center gap-3 px-4 border-b border-ink-200/60">
          ...
          <input
            ...
            className="flex-1 h-12 text-[16px] text-ink-900 placeholder:text-ink-400 bg-transparent border-none outline-none"
          />
