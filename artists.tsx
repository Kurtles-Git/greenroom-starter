...
export const Route = createFileRoute("/artists")({
  head: () => ({ meta: [{ title: "Artists · Greenroom" }] }),
  loader: () => getAllArtists(),
  component: ArtistsPage,
});

type Row = Awaited<ReturnType<typeof getAllArtists>>[number];
...
function Bucket({ title, subtitle, rows }: { title: string; subtitle: string; rows: Row[] }) {
  if (rows.length === 0) return null;
  return (
    <section>
      <div className="flex items-baseline justify-between mb-5">
        <h2 className="font-display text-[24px] font-medium text-ink-900" style={{ letterSpacing: "-0.02em" }}>
          {title}{" "}
            ...
            <div className="flex items-center gap-1.5 mt-3">
              <div className="flex gap-[3px]">
                {Array.from({ length: Math.min(showCount, 8) }).map((_, i) => (
                  <div key={i} className={`w-[5px] h-[5px] rounded-full ${genreColor(artist.genre)}`} />
                ))}
                {showCount > 8 && (
