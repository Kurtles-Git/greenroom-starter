...
import {
  ...
  CardDescription,
  Field,
} from "@/components/greenroom/card";
import { StatusBadge, DealTypeBadge, PlainBadge } from "@/components/greenroom/badge";
import { calculateSettlement } from "@/lib/dealMath";
import {
  buildSettlementStatement,
  buildSettlementTrustAssessment,
  buildRecoveryPayload,
  buildOfframpRecoveryUrl,
  buildOfframpRecoveryFlatUrl,
  type AssumptionState,
} from "@/lib/settlementTrust";

import { DealTrustPanel } from "@/components/settlement/DealTrustPanel";
import { DealTermExtractionCard } from "@/components/settlement/DealTermExtractionCard";
import { TrustFlagList } from "@/components/settlement/TrustFlagList";
import { ScenarioComparison } from "@/components/settlement/ScenarioComparison";
import { AssumptionConfirmation } from "@/components/settlement/AssumptionConfirmation";
import { SettlementStatementPreview } from "@/components/settlement/SettlementStatementPreview";
import { AuditTrail } from "@/components/settlement/AuditTrail";
import { RecoveryModeCard } from "@/components/settlement/RecoveryModeCard";
import { RecoveryPlaybookPreview } from "@/components/settlement/RecoveryPlaybookPreview";
...
export const Route = createFileRoute("/shows/$id/settle")({
  head: () => ({ meta: [{ title: "Settlement · Greenroom" }] }),
  loader: async ({ params }) => {
  ...
  notFoundComponent: () => (
    <div className="px-12 py-10 text-[13px] text-ink-400">Show not found.</div>
  ),
  errorComponent: ({ error }) => (
    <div className="px-12 py-10 text-[13px] text-rose-700">
...
function SettlePage() {
  const data = Route.useLoaderData() as ShowWithRelations;
    ...
    <div className={`px-12 py-10 max-w-7xl ${isDisputed ? "bg-gradient-to-b from-rose-50/30 via-canvas to-canvas" : ""}`}>
      ...
      <div className="mb-20">
        <div className="flex items-center gap-1.5 mb-4">
          ...
          {settlement?.status === "voided" && <PlainBadge variant="default">Voided</PlainBadge>}
        </div>
        ...
        <div className="flex gap-4 items-start max-w-3xl">
          ...
          <div>
            ...
            <p className="text-[13px] text-ink-500 leading-relaxed">
              Greenroom&apos;s in-app settlement tool was built early in the company&apos;s history, when most deals were flat guarantees. About 18% of customers actively use it; the other 82% — including most of the larger venues — default to spreadsheets. The CEO has flagged this as the company&apos;s biggest craft gap.{" "}
              <Link to="/context" className="text-brand-700 font-medium hover:text-brand-800 hover:underline inline-flex items-center gap-0.5">
...
function BackLink({ showId }: { showId: string }) {
  return (
    <Link to="/shows/$id" params={{ id: showId }} className="inline-flex items-center gap-1 text-[12px] text-ink-400 hover:text-ink-900 mb-8 transition-colors">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {isDisputed && (
            <div className="flex items-center gap-1.5 text-[11px] font-medium text-rose-700">
              ...
              {disputedRecoups > 0 && (
                <span className="text-rose-600">· {disputedRecoups} disputed recoup{disputedRecoups === 1 ? "" : "s"}</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-5 gap-1 relative">
          ...
          {stages.map((stage, i) => {
            ...
            const stageDot = isComplete
              ...
              : isCurrent
                ? (isDisputed ? "bg-rose-50 ring-rose-500 text-rose-700" : "bg-brand-50 ring-brand-700 text-brand-700")
      ...
      <Card>
        <CardHeader>
          <div>
            ...
            <CardDescription>
              The inputs Mariana would pull together to settle this show. They&apos;re here — but disconnected from the deal terms.
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent>
          ...
          {deal?.dealNotesFreetext && (
            <div className="mt-6">
              <div className="eyebrow text-[10px] text-ink-500 mb-2">Deal notes (free text — what Mariana actually trusts)</div>
  ...
  return (
    <>
      <div className="text-center py-10 mb-2">
        ...
        <div className="text-[72px] font-mono tabular font-bold text-ink-900 leading-none" style={{ letterSpacing: "-0.03em" }}>
        ...
        {existingSettlement?.totalToArtist != null && existingSettlement.totalToArtist !== calc.totalToArtist && (
          <div className="text-[12px] text-ink-400 mt-2">
            Originally settled at <span className="font-mono tabular text-ink-600">{formatMoney(existingSettlement.totalToArtist)}</span>
          </div>
        )}
      </div>
      ...
      {calc.bonusesNotTriggered.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Structured bonuses on this deal that didn&apos;t hit. Shown for transparency — useful when the agent asks &quot;what about that gross threshold bonus?&quot;
            </CardDescription>
          </CardHeader>
          <CardContent className="divide-y divide-ink-100/80">
...
function RecoupsSection({ recoups }: { recoups: Recoup[] }) {
  ...
  const disputedTotal = recoups.filter((r) => r.status === "disputed").reduce((s, r) => s + r.amount, 0);
    ...
    <Card accent={hasDisputed ? "rose" : undefined}>
      <CardHeader>
        <div>
          ...
          <CardDescription>
            Venue costs taken off the top before artist payment. Often the disputed line items in a settlement.
          </CardDescription>
        </div>
...
function TrustLayerBlock({ data }: { data: ShowWithRelations }) {
  const assessment = useMemo(() => buildSettlementTrustAssessment(data), [data]);
  const [selected, setSelected] = useState<"A" | "B">(assessment.recommendedScenario);
  const [assumptionState, setAssumptionState] = useState<AssumptionState>("unconfirmed");
  const statement = useMemo(
    () => buildSettlementStatement(assessment, selected, assumptionState),
    [assessment, selected, assumptionState],
  );
  const selectedScenario = selected === "A" ? assessment.scenarioA : assessment.scenarioB;

  const recoveryPayload = useMemo(() => buildRecoveryPayload(data, assessment), [data, assessment]);
  const recoveryUrl = useMemo(() => buildOfframpRecoveryUrl(recoveryPayload), [recoveryPayload]);
  const recoveryFlatUrl = useMemo(() => buildOfframpRecoveryFlatUrl(recoveryPayload), [recoveryPayload]);
  const forceTrigger = data.show.id === "show_coastal_spell_dispute";


  return (
    <section className="space-y-6 pt-2">
      <div className="rounded-lg border border-ink-200/80 bg-white px-5 py-4">
        <div className="eyebrow text-[10px] text-ink-500 mb-1">Settlement Trust Layer</div>
        <p className="text-[12.5px] text-ink-700 leading-relaxed max-w-3xl">
          The legacy calculator cannot settle this deal structure. The Trust Layer
          models the ambiguity, shows both payout outcomes, and requires human
          confirmation before finalising the statement.
        </p>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-ink-200/60 bg-canvas-soft px-4 py-3">
          <div className="eyebrow text-[10px] text-ink-500 mb-2">Why this matters</div>
          <ul className="space-y-1.5 text-[12px] text-ink-700 leading-relaxed">
            <li className="flex gap-2">
              <span className="text-brand-700">·</span>
              <span>Deal notes contain the trusted agreement.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-brand-700">·</span>
              <span>The legacy calculator cannot settle this Vs structure.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-brand-700">·</span>
              <span>One ambiguous recoup changes payout by $720.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-brand-700">·</span>
              <span>Greenroom now asks for human confirmation before settlement is sent.</span>
            </li>
          </ul>
        </div>

        <div className="rounded-lg border border-ink-200/60 bg-canvas-soft px-4 py-3">
          <div className="eyebrow text-[10px] text-ink-500 mb-2">AI vs deterministic</div>
          <ul className="space-y-1.5 text-[12px] text-ink-700 leading-relaxed">
            <li className="flex gap-2">
              <span className="text-brand-700">·</span>
              <span>AI-assisted: term extraction, ambiguity detection, explanation draft.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-brand-700">·</span>
              <span>Deterministic: payout math, value at risk, audit trail.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-brand-700">·</span>
              <span>Human-owned: final settlement assumption.</span>
            </li>
          </ul>
        </div>
      </div>

      <DealTrustPanel assessment={assessment} />

      <div className="grid gap-6 lg:grid-cols-2">
        <DealTermExtractionCard terms={assessment.terms} />
        <div className="space-y-3">
          <div className="eyebrow text-[10px] text-ink-500">Trust flags</div>
          <TrustFlagList flags={assessment.flags} />
        </div>
      </div>

      <ScenarioComparison
        scenarioA={assessment.scenarioA}
        scenarioB={assessment.scenarioB}
        selected={selected}
        recommended={assessment.recommendedScenario}
        valueAtRisk={assessment.valueAtRisk}
      ...
      <AssumptionConfirmation
        ...
        onConfirm={() => setAssumptionState("human_confirmed")}
        onReset={() => setAssumptionState("unconfirmed")}
      />

      <SettlementStatementPreview statement={statement} />

      <RecoveryModeCard
        assessment={assessment}
        payload={recoveryPayload}
        recoveryUrl={recoveryUrl}
        recoveryFlatUrl={recoveryFlatUrl}
        forceTrigger={forceTrigger}
      />

      <RecoveryPlaybookPreview assessment={assessment} payload={recoveryPayload} forceTrigger={forceTrigger} />
