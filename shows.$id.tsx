...
export const Route = createFileRoute("/shows/$id")({
  head: () => ({ meta: [{ title: "Show · Greenroom" }] }),
  loader: async ({ params }) => {
...
function ShowDetailPage() {
  ...
  const {
    show, artist, agent, agency, deal, settlement, ticketSales, expenses, comps,
  } = data;

  if (pathname !== `/shows/${show.id}`) return <Outlet />;
      ...
      <div className={`px-12 pt-10 pb-14 ${isDisputed ? "bg-gradient-to-b from-rose-50/40 to-canvas" : "bg-gradient-to-b from-brand-50/30 to-canvas"}`}>
        ...
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <h1 className="font-display text-[56px] font-medium text-ink-900 leading-[1.02]" style={{ letterSpacing: "-0.025em", fontOpticalSizing: "auto" }}>
              {artist?.name ?? "—"}
            </h1>
          ...
          <Link to="/shows/$id/settle" params={{ id: show.id }} className="mt-6 shrink-0">
      ...
      <div className="px-12 pb-12">
        {show.internalNotes && (
          <div className="mb-8 mt-1 rounded-lg bg-amber-50/50 ring-1 ring-amber-200/60 p-5 flex gap-3">
            ...
            <div>
              <div className="eyebrow text-[10px] text-amber-800 mb-1.5">Mariana&apos;s notes</div>
              <div className="text-[13px] text-ink-800 leading-relaxed">{show.internalNotes}</div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-2">
                  ...
                  {bonuses.length > 0 && (
                    <div className="rounded-lg ring-1 ring-brand-200/50 bg-brand-50/20 p-4">
                      <div className="flex items-center gap-1.5 mb-2.5">
                        ...
                        <div className="eyebrow text-[10px] text-brand-800">Bonuses & escalators (structured)</div>
                      </div>
                      ...
                      <div className="text-[11px] text-ink-400 mt-3 leading-snug">
                        Stored in <code className="font-mono text-[10px] bg-white/80 px-1 py-0.5 rounded ring-1 ring-ink-200/40">bonuses_json</code>. The in-app tool only reads structured bonuses — anything in the prose below is invisible to it.
                      </div>
                    </div>
                  )}
          ...
          <Card>
            <CardHeader><CardTitle>Artist & agent</CardTitle></CardHeader>
              ...
              <div className="space-y-3">
                ...
                {totalTickets > 0 ? (
                  <div className="text-[12px] text-ink-500 pt-4 border-t border-ink-100/80 leading-relaxed">
                    <span className="font-mono tabular font-medium text-ink-700">{totalTickets}</span> tickets ·{" "}
                    <span className="font-mono tabular">{formatMoney(totalFees)}</span> in fees
                    <div className="mt-1.5 text-ink-400">
                      Net <span className="font-mono tabular text-ink-700">{formatMoneyCompact(grossSoFar - totalFees)}</span>
                    </div>
                  </div>
                ) : (
                  <div className="text-[12px] text-ink-400 pt-3 border-t border-ink-100/80">No sales yet.</div>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent>
              {comps.length === 0 ? (
                <div className="text-[13px] text-ink-400">No comps recorded for this show.</div>
              ) : (
          ...
          <Card className="md:col-span-3">
            <CardHeader>
              ...
              {absorbedTotal > 0 && <PlainBadge variant="amber">{formatMoney(absorbedTotal)} absorbed</PlainBadge>}
            </CardHeader>
            <CardContent>
              {expenses.length === 0 ? (
